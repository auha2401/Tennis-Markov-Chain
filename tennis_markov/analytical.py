...
def p_win_tiebreak(p: float) -> float:
    """Probability of winning a tiebreak (first to 7 points, 2-point margin)."""
    q = 1 - p
...
def p_win_set(p: float) -> float:
    """Probability of winning a set (first to 6 games, tiebreak at 6-6)."""
    pg = p_win_game(p)
...
def p_win_match(p: float, best_of: int = 3) -> float:
    """Probability of winning a best-of-N sets match."""
    ps = p_win_set(p)
    needed = (best_of + 1) // 2
    total = 0.0
